...
RSpec.describe "real API selector execution", :real_api do
  ...
  it "executes resolved selectors against a kind cluster and records coverage" do
    ...
    expect(summary.fetch("unsupported")).to eq(0), "Expected zero executor-unsupported targets, got #{summary.fetch('unsupported')}: #{JSON.pretty_generate(result)}"
    expect(summary.fetch("covered") + summary.fetch("unavailable")).to eq(summary.fetch("recorded"))
