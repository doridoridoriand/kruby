...
RSpec.describe "full mode core/v1 service accounts coverage" do
  it "contains CRUD selectors for core/v1 service accounts" do
    ...
    expected_ops = %w[create get list update patch delete].map do |op|
      "core/v1/service-accounts:#{op}"
    end

    expect(selection.mode).to eq("full")
    expect(selection.resolved_targets).to include(*expected_ops)
    expect(selection.resolved_targets.index("core/v1/service-accounts:create"))
      .to be < selection.resolved_targets.index("core/v1/service-accounts:delete")
  end
end
