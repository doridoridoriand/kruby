  ...
  class V2HPAScalingRules
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V2HPAScalingRules` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V2HPAScalingRules`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'select_policy')
        self.select_policy = attributes[:'select_policy']
      end

      if attributes.key?(:'stabilization_window_seconds')
        self.stabilization_window_seconds = attributes[:'stabilization_window_seconds']
      end

      if attributes.key?(:'tolerance')
        self.tolerance = attributes[:'tolerance']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          policies == o.policies &&
          select_policy == o.select_policy &&
          stabilization_window_seconds == o.stabilization_window_seconds &&
    ...
    def hash
      [policies, select_policy, stabilization_window_seconds, tolerance].hash
    end
