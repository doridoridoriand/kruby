  ...
  class V1AuditAnnotation
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1AuditAnnotation` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1AuditAnnotation`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'value_expression')
        self.value_expression = attributes[:'value_expression']
      end
    end
    ...
    def list_invalid_properties
      ...
      if @value_expression.nil?
        invalid_properties.push('invalid value for "value_expression", value_expression cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @value_expression.nil?
      true
    end
