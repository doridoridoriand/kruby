  ...
  class V1alpha3ResourcePoolStatusRequestStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1alpha3ResourcePoolStatusRequestStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1alpha3ResourcePoolStatusRequestStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @pool_count.nil?
        invalid_properties.push('invalid value for "pool_count", pool_count cannot be nil.')
      end
    ...
    def valid?
      return false if @pool_count.nil?
      true
    end
    ...
    def hash
      [conditions, pool_count, pools].hash
    end
