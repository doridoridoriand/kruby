  ...
  class V1VolumeMountStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1VolumeMountStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1VolumeMountStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'recursive_read_only')
        self.recursive_read_only = attributes[:'recursive_read_only']
      end

      if attributes.key?(:'volume_status')
        self.volume_status = attributes[:'volume_status']
      end
    end
    ...
    def hash
      [mount_path, name, read_only, recursive_read_only, volume_status].hash
    end
