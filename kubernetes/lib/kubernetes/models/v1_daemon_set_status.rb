  ...
  class V1DaemonSetStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1DaemonSetStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1DaemonSetStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'number_available')
        self.number_available = attributes[:'number_available']
      end

      if attributes.key?(:'number_misscheduled')
        self.number_misscheduled = attributes[:'number_misscheduled']
      end

      if attributes.key?(:'number_ready')
        self.number_ready = attributes[:'number_ready']
      end

      if attributes.key?(:'number_unavailable')
        self.number_unavailable = attributes[:'number_unavailable']
      end
      ...
      if attributes.key?(:'updated_number_scheduled')
