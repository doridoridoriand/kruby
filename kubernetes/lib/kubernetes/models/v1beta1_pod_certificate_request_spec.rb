...
module Kubernetes
  ...
  class V1beta1PodCertificateRequestSpec
    ...
    attr_accessor :stub_pkcs10_request
    ...
    # Custom attribute writer method with validation
    # @param [Object] stub_pkcs10_request Value to be assigned
    def stub_pkcs10_request=(stub_pkcs10_request)
      if stub_pkcs10_request.nil?
        fail ArgumentError, 'stub_pkcs10_request cannot be nil'
      end

      pattern = Regexp.new(/^(?:[A-Za-z0-9+\/]{4})*(?:[A-Za-z0-9+\/]{2}==|[A-Za-z0-9+\/]{3}=)?$/)
      if stub_pkcs10_request !~ pattern
        fail ArgumentError, "invalid value for \"stub_pkcs10_request\", must conform to the pattern #{pattern}."
      end

      @stub_pkcs10_request = stub_pkcs10_request
    end
