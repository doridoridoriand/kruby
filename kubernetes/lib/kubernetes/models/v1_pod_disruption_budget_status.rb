  ...
  class V1PodDisruptionBudgetStatus
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1PodDisruptionBudgetStatus` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1PodDisruptionBudgetStatus`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'current_healthy')
        self.current_healthy = attributes[:'current_healthy']
      end

      if attributes.key?(:'desired_healthy')
        self.desired_healthy = attributes[:'desired_healthy']
      end

      if attributes.key?(:'disrupted_pods')
        if (value = attributes[:'disrupted_pods']).is_a?(Hash)
          self.disrupted_pods = value
        end
      end

      if attributes.key?(:'disruptions_allowed')
        self.disruptions_allowed = attributes[:'disruptions_allowed']
      end

      if attributes.key?(:'expected_pods')
        self.expected_pods = attributes[:'expected_pods']
      end
    ...
    def hash
      [conditions, current_healthy, desired_healthy, disrupted_pods, disruptions_allowed, expected_pods, observed_generation].hash
    end
