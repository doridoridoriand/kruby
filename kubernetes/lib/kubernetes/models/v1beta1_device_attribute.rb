  ...
  class V1beta1DeviceAttribute
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1beta1DeviceAttribute` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1beta1DeviceAttribute`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'bools')
        if (value = attributes[:'bools']).is_a?(Array)
          self.bools = value
        end
      end
      ...
      if attributes.key?(:'ints')
        if (value = attributes[:'ints']).is_a?(Array)
          self.ints = value
        end
      end
      ...
      if attributes.key?(:'strings')
        if (value = attributes[:'strings']).is_a?(Array)
          self.strings = value
        end
      end
    ...
    def hash
      [bool, bools, int, ints, string, strings, version, versions].hash
    end
