  ...
  class V1beta1ResourcePool
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1beta1ResourcePool` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1beta1ResourcePool`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @resource_slice_count.nil?
        invalid_properties.push('invalid value for "resource_slice_count", resource_slice_count cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @resource_slice_count.nil?
      true
    end
    ...
    def hash
      [generation, name, resource_slice_count].hash
    end
