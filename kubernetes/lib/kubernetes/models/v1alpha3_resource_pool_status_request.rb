  ...
  class V1alpha3ResourcePoolStatusRequest
    ...
    def self.openapi_types
      {
        ...
        :'spec' => :'V1alpha3ResourcePoolStatusRequestSpec',
        :'status' => :'V1alpha3ResourcePoolStatusRequestStatus'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1alpha3ResourcePoolStatusRequest` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1alpha3ResourcePoolStatusRequest`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
