...
module Kubernetes
  ...
  class V1PodSchedulingGroup
    ...
    attr_accessor :pod_group_name

    # Attribute mapping from ruby-style variable name to JSON key.
    def self.attribute_map
      {
        :'pod_group_name' => :'podGroupName'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Kubernetes::V1PodSchedulingGroup` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Kubernetes::V1PodSchedulingGroup`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }

      if attributes.key?(:'pod_group_name')
        self.pod_group_name = attributes[:'pod_group_name']
      end
    end
    ...
    def ==(o)
      ...
      self.class == o.class &&
          pod_group_name == o.pod_group_name
    end
