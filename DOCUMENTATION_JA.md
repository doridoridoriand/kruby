...
```bash
git clone https://github.com/doridoridoriand/kruby.git
cd kruby
```
...
正常にインストールされていれば、バージョン番号（例: `1.36.0.2`）が表示されます。
...
3. **生成されたコードの確認**
   ...
   リリース前に以下のバージョン整合性を確認してください：
   - `Kubernetes::VERSION` の値が、生成したクライアントのバージョンと一致しているか
   - gem 名（例: `kruby-1.36.0.2.gem`）に埋め込まれたバージョン文字列が一致しているか
   - 文書内（`DOCUMENTATION_JA.md` / `README.md`）のバージョン参照（「クライアントバージョン」「Compatibility matrix」テーブル、インストール例など）がリリース対象バージョンと一致しているか
   - `Kubernetes::VERSION` 定数と gem バージョンの不一致があると、インストール後にも古いバージョンが表示されるため、必ずクロスチェックを行ってください
...
[https://github.com/doridoridoriand/kruby/issues](https://github.com/doridoridoriand/kruby/issues)
