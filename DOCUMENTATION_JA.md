...
|------|-----------|-------------------|-----------------|
| **Ruby** | doridoridoriand/k8sruby | 1.31 | コミュニティメンテナンス |
...
これにより、`kubernetes-1.31.0.gem`ファイルが生成されます。
...
```bash
ruby -e "require 'kubernetes'; puts Kubernetes::VERSION"
```

正常にインストールされていれば、バージョン番号（例: `1.31.0`）が表示されます。
